...
def dump_frames(vid_path):
    ...
    if video_id in video_dic.keys():
        ...
        if len(file_list)>7:
            ...
            # if video_type=='val':
            #     val_video_path.append(out_full_path+' '+str(len(file_list))+' '+str(label_index)+'\n')    
...
if __name__ == '__main__':
    ...
    parser.add_argument("--src_dir",type=str,default='/media/sdb_access/Kinetics/k_400_data_set')
    parser.add_argument("--out_dir", type=str,default='/media/sdb_access/Data_for_Action_CLIP/KINETICS_400')
    ...
    with open("/media/sda1_acces/Code/ActionCLIP_without_wandb/ActionCLIP/lists/kinetics_400_labels.csv", 'r') as file:
        csvreader = csv.reader(file)
        for i,row in enumerate(csvreader):
            if i>0:
                index = int(row[0])
    ...
    # writing to file
    file1 = open('/media/sda1_acces/Code/ActionCLIP_without_wandb/train_video.txt', 'w')
    file1.writelines(list(train_video_path))
    file1.close()

    file2 = open('/media/sda1_acces/Code/ActionCLIP_without_wandb/test_video.txt', 'w')
    file2.writelines(list(test_video_path))
    file2.close()

    file2 = open('/media/sda1_acces/Code/ActionCLIP_without_wandb/val_video.txt', 'w')
