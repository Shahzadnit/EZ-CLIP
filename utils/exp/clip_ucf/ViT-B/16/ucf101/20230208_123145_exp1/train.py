...
sys.path.insert(0, '/media/sda1_acces/Code/VPT_new')
from train_exp import Prompt_Vit_model
################################################################
from modules.Visual_Prompt import visual_prompt
from utils.KLLoss import KLLoss
from test import validate,validate_new
from utils.Augmentation import *
from utils.solver import _optimizer, _optimizer_2, _lr_scheduler
...
def main():
    ...
    parser.add_argument('--config', '-cfg', default='/home/shahzaa/Code/ActionCLIP/configs/ucf101/ucf_train.yaml')
    parser.add_argument('--prompt-vit-config-file', default='/media/sda1_acces/Code/VPT_new/cars.yaml',metavar="FILE")
    parser.add_argument('--log_time', default='')
    args = parser.parse_args()
    args.log_time='20230208_123145_exp1'
    # print('##################################',args.log_time)
    ...
    shutil.copy('/media/sda1_acces/Code/ActionCLIP_without_wandb/ActionCLIP/train.py', working_dir)
    ...
    for k, p in model_text.named_parameters():
        # print(k)
        if "Adapter" not in k:
            p.requires_grad = False
    for k, p in model_image.named_parameters():
        # print(k)
        if "Adapter" not in k:
            p.requires_grad = False
    ###########################################################
    ...
    parameters = filter(lambda p: p.requires_grad, model_image.parameters())
    parameters = sum([np.prod(p.size()) for p in parameters]) / 1_000_000
    print('model_image Trainable Parameters: %.3fM' % parameters)

    parameters = filter(lambda p: p.requires_grad, model_text.parameters())
    parameters = sum([np.prod(p.size()) for p in parameters]) / 1_000_000
    print('model_text  Trainable Parameters: %.3fM' % parameters)

    
    parameters = filter(lambda p: p.requires_grad, prompt_vit_model.parameters())
    parameters = sum([np.prod(p.size()) for p in parameters]) / 1_000_000
    print('prompt_vit_model Trainable Parameters: %.3fM' % parameters)
    ############################################################
    ...
    for epoch in range(start_epoch, config.solver.epochs):
        ...
        for kkk,(images,list_id) in enumerate(tqdm(train_loader)):
            ...
            # image_embedding = prompt_vit_model(images)
        ...
        epoch_saving_1(epoch, model, fusion_model, optimizer, filename)
        if is_best:
            best_saving_1(working_dir, epoch, model, fusion_model, optimizer)
