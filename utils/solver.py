...
def _optimizer_1(config, model, vit_model, fusion_model):
    if config.solver.optim == 'adam':
        optimizer = optim.Adam([{'params': model.parameters()},{'params': vit_model.parameters(), 'lr': config.solver.lr * config.solver.f_ratio},  
         {'params': fusion_model.parameters(), 'lr': config.solver.lr * config.solver.f_ratio}],
                               lr=config.solver.lr, betas=(0.9, 0.98), eps=1e-8,
                               weight_decay=0.2)  # Params used from paper, the lr is smaller, more safe for fine tuning to new dataset
        print('Adam')
        ...
        optimizer = optim.SGD([{'params': model.parameters()}, {'params': vit_model.parameters(), 'lr': config.solver.lr * config.solver.f_ratio},  
         {'params': fusion_model.parameters(), 'lr': config.solver.lr * config.solver.f_ratio}],
                              config.solver.lr,
                              momentum=config.solver.momentum,
                              weight_decay=config.solver.weight_decay)
        ...
        optimizer = optim.AdamW([{'params': text_params},
                                 {'params': vit_model.parameters(), 'lr': config.solver.lr * config.solver.ratio},
                                 {'params': fusion_model.parameters(), 'lr': config.solver.lr * config.solver.f_ratio}],
                                betas=(0.9, 0.98), lr=config.solver.lr, eps=1e-8,
                                weight_decay=config.solver.weight_decay)  # Params used from paper, the lr is smaller, more safe for fine tuning to new dataset
        for param_group in optimizer.param_groups:
...
def _optimizer_2(config, model_image, model_text, fusion_model):
    # if config.solver.optim == 'adam':
    #     optimizer = optim.Adam([{'params': model.parameters()},  
    #      {'params': fusion_model.parameters(), 'lr': config.solver.lr * config.solver.f_ratio}],
    #                            lr=config.solver.lr, betas=(0.9, 0.98), eps=1e-8,
    ...
    if config.solver.optim == 'adamw':
        vision_params = list(map(id, model_image.parameters()))
        text_params = list(map(id, model_text.parameters()))

        optimizer = optim.AdamW([{'params': text_params},
                                 {'params': vision_params , 'lr': config.solver.lr * config.solver.ratio},
                                 {'params': fusion_model.parameters(), 'lr': config.solver.lr * config.solver.f_ratio}],
                                betas=(0.9, 0.98), lr=config.solver.lr, eps=1e-8,
                                weight_decay=config.solver.weight_decay)  # Params used from paper, the lr is smaller, more safe for fine tuning to new dataset
        for param_group in optimizer.param_groups:
