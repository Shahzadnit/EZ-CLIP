...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
        ...
        # output, atten = self.attn(x, x, x, need_weights=True, attn_mask=self.attn_mask)
    ...
    def forward(self, x: torch.Tensor, T_prompt=None, Text_prompt=None, layer_num=None, return_attention=False):
        if self.model_for =='image':
            ...
            if T_prompt is not None:
                ...
                # T_prompt = T_prompt.expand(-1, x.shape[1],-1,-1)
                # T_prompt = torch.mean(torch.mul(T_prompt ,x),0)
                
                
                # T_prompt = T_prompt.expand(x.shape[1],-1,-1) + x[0,:,:,:]         #torch.mean(x, 0)   T_prompt = T_prompt.expand(x.shape[1],-1,-1) + x[0,:,:,:]
                T_prompt = T_prompt.expand(x.shape[1],-1,-1) + torch.mean(x, 0)
                # T_prompt = torch.mul(T_prompt.expand(x.shape[1],-1,-1), torch.mean(x, 0))
                
                
                
                T_prompt = T_prompt.view(b, self.T, 1, d) 
                T_prompt = T_prompt.permute(1,2,0,3).view(self.T, b, d) 
                # T_prompt = T_prompt + self.drop_path(self.attention(self.ln_1(T_prompt)))
...
#     def prompt_attention(self, x: torch.Tensor):
#         self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
#         return self.prompt_attn(x, x, x, need_weights=False, attn_mask=self.attn_mask)[0]
 

#     def forward(self, x: torch.Tensor, S_prompt=None,T_prompt=None, Text_prompt=None, layer_num=None):
#         if self.model_for =='image':
#             l, bt, d = x.size()          
#             if T_prompt is not None:
#                 b = bt // self.T
...
#                 T_prompt = T_prompt.view(b, self.T, 1, d) 
#                 T_prompt = T_prompt.permute(1,2,0,3).view(self.T, b, d) 
#                 T_prompt = T_prompt + self.drop_path(self.prompt_attention(self.ln_1(T_prompt)))
#                 T_prompt = T_prompt.view(self.T, 1, b, d).permute(1,2,0,3)
...
#             #################################################################       
#             num_of_token = self.config.prompt.num_of_token
#             if S_prompt is not None:
#                 ############# Spatial Prompting #####################
#                 if layer_num==0:
#                     x = torch.cat((x[:1, :, :],S_prompt.expand(-1,x.shape[1],-1),x[1:, :, :]),dim=0)
#                 else:
#                     x = torch.cat((x[:1, :, :],S_prompt.expand(-1,x.shape[1],-1),x[(1+num_of_token):, :, :]),dim=0)
#                 x = x + self.drop_path(self.attention(self.ln_1(x)))
#                 x = x[:l+num_of_token,:,:]
#                 x = x + self.drop_path(self.mlp(self.ln_2(x)))
#                 return x
...
#                 return x
#         if self.model_for =='text':
#             num_of_token = self.config.prompt.num_of_text_token
#             if Text_prompt is not None:
#                 if layer_num==0:
#                     x = torch.cat((x[:1, :, :],Text_prompt.expand(-1,x.shape[1],-1),x[1:, :, :]),dim=0)
#                 else:
#                     x = torch.cat((x[:1, :, :],Text_prompt.expand(-1,x.shape[1],-1),x[(1+num_of_token):, :, :]),dim=0)
#                 x = x + self.drop_path(self.attention(self.ln_1(x)))
#                 x = x + self.drop_path(self.mlp(self.ln_2(x)))
#                 return x
#             else:    
        ...
        if self.model_for == 'image':
            if config.prompt.use:
                ...
                if config.prompt.INITIATION == "random":
                    # self.T_prompt_embeddings = nn.Parameter(torch.randn(197,self.layers, self.no_frame, self.width))
                    ...
                    # self.T_prompt_embeddings = nn.Parameter(torch.randn(197,self.layers, self.no_frame, 1))
                    # self.T_prompt_embeddings = nn.Parameter(scale * torch.randn(self.layers, 1, self.no_frame, self.width))
                    nn.init.uniform_(self.T_prompt_embeddings.data, -val, val)
                    # nn.init.normal_(self.T_prompt_embeddings, std=0.02)
                else:
