...
from test import validate,validate_new
...
def main():
    ...
    parser.add_argument('--config', '-cfg', default='/media/sda1_acces/Code/ActionCLIP_without_wandb/Prompt_Adaptor_mix_work/ActionCLIP/configs/ucf101/ucf_train.yaml')
    ...
    args.log_time='Chat_gpt_testing'
    # print('##################################',args.log_time)
    ...
    shutil.copy('/media/sda1_acces/Code/ActionCLIP_without_wandb/Prompt_Adaptor_mix_work/ActionCLIP/configs/ucf101/ucf_train.yaml', working_dir)
    ...
    k_val_data = Action_DATASETS(config.data.k_val_list,config.data.k_label_list, random_shift=False,num_segments=config.data.num_segments,image_tmpl=config.data.image_tmpl,
                       transform=transform_val)
    k_val_loader = DataLoader(k_val_data,batch_size=config.data.batch_size,num_workers=config.data.workers,shuffle=False,pin_memory=False,drop_last=True)
    ###################################################################################################################################################
    ...
    ############################## UCF-101 test loader ###################################
    ucf_train_data = Action_DATASETS(config.data.ucf_train_list,config.data.ucf_label_list,num_segments=config.data.num_segments,image_tmpl=config.data.image_tmpl,random_shift=config.data.random_shift,
                        transform=transform_train)
    ucf_train_loader = DataLoader(ucf_train_data,batch_size=config.data.batch_size,num_workers=config.data.workers,shuffle=True,pin_memory=False,drop_last=True)
    ...
    k_classes, k_num_text_aug, k_text_dict = text_prompt(k_val_data,config.data.k_gpt_discription, config.data.use_chat_gpt)
    
    ##### UCF ########
    ucf_classes, ucf_num_text_aug, ucf_text_dict = text_prompt(ucf_train_data,config.data.ucf_gpt_discription,config.data.use_chat_gpt)
    ...
    if config.solver.evaluate:
        ucf_prec1 = validate(start_epoch,ucf_val_loader, ucf_classes, device,model, fusion_model, config,ucf_num_text_aug,working_dir,'ucf')
        return
    ...
    for epoch in range(start_epoch, config.solver.epochs):
        ...
        tic = time.time()
        epoch_loss=[]
        for kkk,(prompt_images,list_id) in enumerate((ucf_train_loader)):
            if config.solver.type != 'monitor':
                if (kkk+1) == 1 or (kkk+1) % 10 == 0:
                    lr_scheduler.step(epoch + kkk / len(ucf_train_loader))
            optimizer.zero_grad()
            ...
            text_id = numpy.random.randint(ucf_num_text_aug,size=len(list_id))
            texts = torch.stack([ucf_text_dict[j][i,:] for i,j in zip(list_id,text_id)])
            ...
            if kkk%10==0:    
                print('Epoch:%d  iteration:%d/%d, total loss:%f, image loss:%f, text loss:%f, motion loss:%f, lr:%f '%(epoch,kkk,len(ucf_train_loader),total_loss.item(),loss_imgs.item(),loss_texts.item(), loss_video_motion.item(), optimizer.param_groups[0]['lr']))
        ...
        if epoch % config.logging.eval_freq == 0:  # and epoch>0
            ...
            k_prec1 = validate(epoch,k_val_loader, k_classes, device, model, fusion_model, config,k_num_text_aug, working_dir,'K-400')
        # print('Epoch:%d, total loss:%f, image loss:%f, text loss:%f'%(epoch,total_loss.item(),loss_imgs.item(),loss_texts.item()))
        k_is_best = k_prec1 > k_best_prec1
        k_best_prec1 = max(k_prec1, k_best_prec1)
        print('K-400 Testing: {}/{}'.format(k_prec1,k_best_prec1))
        ...
        if os.path.exists(txt_path):
            with open(txt_path, 'a+') as f:
                f.write('\n')
                f.write('Epoch:%d  iteration:%d/%d, total loss:%f, image loss:%f, text loss:%f, motion loss:%f, lr:%f \n'%(epoch,kkk,len(ucf_train_loader),total_loss.item(),loss_imgs.item(),loss_texts.item(), loss_video_motion.item(), optimizer.param_groups[0]['lr']))
                f.write('K-400 Testing: {}/{}\n'.format(k_prec1,k_best_prec1))
        ...
        k_top_1_acc.append(k_prec1/100)
        ...
        plt.plot(X, k_top_1_acc, color='g', label='K-400 Accuracy')
        plt.plot(X, ucf_top_1_acc, color='b', label='UCF-101 Accuracy')
        plt.plot(X, hmdb_top_1_acc, color='m', label='HMDB-51 Accuracy')
        # plt.plot(X, something_top_1_acc, color='m', label='HMDB-51 Accuracy')
        plt.xlabel("Epoch")
