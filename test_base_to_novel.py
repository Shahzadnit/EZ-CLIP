...
def harmonic_mean(num1, num2):
    # Check if the numbers are non-zero to avoid division by zero
    if num1 == 0 or num2 == 0:
        raise ValueError("Harmonic mean is undefined when one or both numbers are zero.")
...
def main():
    ...
    novel_val_data = Action_DATASETS(config.data.novel_val_list, config.data.novel_label_list, num_segments=config.data.num_segments,
                        image_tmpl=config.data.image_tmpl,
                        transform=transform_val, random_shift=config.random_shift)
    novel_val_loader = DataLoader(novel_val_data, batch_size=config.data.batch_size, num_workers=config.data.workers, shuffle=False,
                            pin_memory=True, drop_last=True)
    

    base_val_data = Action_DATASETS(config.data.val_list, config.data.label_list, num_segments=config.data.num_segments,
                        image_tmpl=config.data.image_tmpl,
                        transform=transform_val, random_shift=config.random_shift)
    base_val_loader = DataLoader(base_val_data, batch_size=config.data.batch_size, num_workers=config.data.workers, shuffle=False,
                            pin_memory=True, drop_last=True)
    ...
    novel_classes, novel_num_text_aug, novel_text_dict = text_prompt(novel_val_data,config.data.gpt_discription,config.data.use_llm)

    base_classes, base_num_text_aug, base_text_dict = text_prompt(base_val_data,config.data.gpt_discription,config.data.use_llm)

    best_prec1 = 0.0
    # 'UCF' is dataset name, change according to dataset
    novel_prec1 = validate(start_epoch, novel_val_loader, novel_classes, device, model, config, novel_num_text_aug, working_dir,'UCF_novel')
    base_prec1 = validate(start_epoch, base_val_loader, base_classes, device, model, config, base_num_text_aug, working_dir,'UCF_base')
    print('Base: {}, Novel: {}, HM: {}'.format(base_prec1, novel_prec1, harmonic_mean(base_prec1, novel_prec1)))
