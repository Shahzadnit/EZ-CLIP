...
def validate_new(epoch, val_loader, classes, device, model, vit_model, fusion_model, config, num_text_aug):
    vit_model.eval()
    fusion_model.eval()
    model.eval()
    num = 0
    ...
    with torch.no_grad():
        ...
        for iii, (image, class_id) in enumerate(tqdm(val_loader)):
            ...
            image_features = vit_model(image_input).view(b, t, -1)
            image_features = fusion_model(image_features)
            image_features /= image_features.norm(dim=-1, keepdim=True)
            text_features /= text_features.norm(dim=-1, keepdim=True)
            similarity = (100.0 * image_features @ text_features.T)
...
def main():
    ...
    shutil.copy('/media/shahzaa/Extra/Code/ActionCLIP_without_wandb/prompt_work/ActionCLIP/test.py', working_dir)
    ...
    classes, num_text_aug, text_dict = text_prompt(val_data,config.data.gpt_discription)
