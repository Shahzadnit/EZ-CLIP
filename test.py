...
def validate_1(epoch, val_loader, classes, device, model, fusion_model, config, num_text_aug,working_dir,dataset_name):
    model.eval()
    fusion_model.eval()
    num = 0
    corr_1 = 0
...
def main():
    ...
    args.log_time='SSv2_base_to_novel_testing'
    with open(args.config, 'r') as f:
        config = yaml.load(f)
    working_dir = os.path.join('/media/sda1_acces/Code/ActionCLIP_without_wandb/Prompt_Adaptor_mix_work/Exp', config['network']['type'], config['network']['arch'], config['data']['dataset'],
                               args.log_time)
    ...
    best_prec1 = 0.0
    prec1 = validate_1(start_epoch, val_loader, classes, device, model, fusion_model, config, num_text_aug,working_dir,'UCF_base')
...
# image = cv2.imread('/home/shahzaa/Downloads/srk.png')
# Attn_map = self.ln_post(x[:, 1:, :]).view(8,14,14,768)
# for i,attention_tensor in enumerate(Attn_map[:,]):
...
#     overlay = cv2.addWeighted(image, 0.7, heatmap_colormap, 0.3, 0)
#     cv2.imwrite('/media/sda1_acces/Code/ActionCLIP_without_wandb/Prompt_Adaptor_mix_work/heatMap_%d.png'%(i), overlay)